...
const uploadedFilesStore = new Map<number, UploadedRecord>();
...
const checkS3Availability = async (
  ...
  // If no bucket configured, use mock mode (all files available for demo)
...
// In-memory job queue (simple fallback for demo/dev).
// For production, replace with Redis + BullMQ worker.
type JobStatus = "queued" | "processing" | "ready" | "failed" | "cancelled";
...
async function processJobFromQueue(): Promise<void> {
  ...
  try {
    for (let i = job.processed; i < job.total; i++) {
      const fileId = job.fileIds[i];

      // simulate processing delay per file
      const delayMs = getRandomDelay();
      await sleep(delayMs);

      // For demo mode generate an internal download URL that the frontend can call
      // Use the API route that serves uploaded files. This avoids external DNS.
      job.resultUrls[i] = `/v1/files/${fileId}/download`;
...
// Start background job processor using setInterval
setInterval(() => {
  processJobFromQueue().catch((err: unknown) => {
...
const uploadRoute = createRoute({
  ...
  description: "Upload files to storage and get file IDs",
  responses: {
    200: {
...
app.openapi(uploadRoute, async (c) => {
  ...
  if (!files || files.length === 0) {
    return c.json({ error: "Bad request", message: "No files provided" }, 400);
  }
  ...
  for (const file of files) {
    // file is a File-like object from the form data
    ...
    uploadedFiles.push({ fileId, fileName, uploadedAt });
  }

  return c.json({ files: uploadedFiles }, 200);
});

// Serve uploaded file bytes (demo endpoint)
const fileDownloadRoute = createRoute({
  method: "get",
  path: "/v1/files/:fileId/download",
  tags: ["Files"],
  summary: "Download uploaded file",
  description: "Returns the uploaded file content for demo purposes",
  responses: {
    200: {
...
app.openapi(fileDownloadRoute, (c) => {
  const id = parseInt(c.req.param("fileId"), 10);
  if (Number.isNaN(id)) {
    return c.json({ error: "Bad request", message: "Invalid file id" }, 400);
  }
  const rec = uploadedFilesStore.get(id);
  if (!rec) {
    return c.json({ error: "Not found", message: "File not found" }, 404);
  }

  c.header("Content-Type", rec.mime || "application/octet-stream");
  c.header("Content-Disposition", `attachment; filename="${rec.fileName}"`);
  // cast to any so TypeScript typings for c.body accept Buffer
  return c.body(rec.data as any, 200);
});
...
app.openapi(downloadInitiateRoute, (c) => {
  ...
  const job: JobRecord = {
    ...
    resultUrls: Array.from({ length: file_ids.length }, () => null),
    error: null,
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
...
const downloadStatusRoute = createRoute({
  ...
  description: "Returns status and progress for a given jobId",
  responses: {
    200: {
      ...
      content: {
        "application/json": {
          schema: z.object({
            ...
            processed: z.number().int().nullable(),
            total: z.number().int().nullable(),
            resultUrls: z.array(z.string().nullable()).nullable(),
  ...
  return c.json(
    {
      ...
      total: job.total,
      // Normalize any external demo URLs to internal API download routes so
      // the frontend can download files even if they were created before
      // we switched to internal URLs.
      resultUrls: (job.resultUrls || []).map((u) => {
        try {
          if (!u) return null;
          // If url is already internal (starts with /v1), return as-is
          if (u.startsWith("/v1/")) return u;
          const parsed = new URL(u);
          // storage.example.com/downloads/<id>.zip?token=...
          if (parsed.hostname === "storage.example.com") {
            const parts = parsed.pathname.split("/").filter(Boolean);
            const last = parts[parts.length - 1] || ""; // e.g., 100003.zip
    ...
    return c.json(
      {
        ...
        // Return internal download URL (frontend will proxy /v1 to backend)
        downloadUrl: `/v1/files/${file_id}/download`,
        size: s3Result.size,
        processingTimeMs,
